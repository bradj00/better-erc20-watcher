...
var sleepSeconds = 300000; // 5 minutes between each run
// var sleepSeconds = 5;


console.clear();
console.log(chalk.cyan.underline.inverse('translator-ens.js')+'\n');
...
function checkIfAddressesExistInFriendlyNames( uniqueAddys ){
    ...
    return new Promise(async (resolve, reject) => {
        ...
        for (const address of uniqueAddys) {
            ...
            if (collectionExists && typeof collectionExists[0] == 'object' && collectionExists[0].ENS) {
            ...
            return promiseChain.then(() => {
                ...
                if (index == ListOfAddresses.length - 1) {
                    ...
                    MongoClient.connect(mongoUrl, function(err, client) {
                        ...
                        var d = new Date();
                        db.collection("systemStatuses").updateOne({name:"translator-ens"}, {$set:{name:"translator-ens", lookupIndex: 0, lookupIndexMax: 0, statusMsg: "indexing new addresses against external API", lastAction: d}},{upsert: true},  function(err, result) {
                ...
                MongoClient.connect(mongoUrl, function(err, client) {
                    ...
                    const db = client.db('systemStats');
                    var d = new Date();
                    db.collection("systemStatuses").updateOne({name:"translator-ens"}, {$set:{name:"translator-ens", lookupIndex: index+1, lookupIndexMax: ListOfAddresses.length, statusMsg: "indexing new addresses against external API", lastAction: d}},{upsert: true},  function(err, result) {
...
const LookupSingleAddress =  (singleAddress, count, totalCount) => {
    return new Promise( (resolve, reject) => {
        setTimeout(async() => {
            // singleAddress = "0xe7ded2ebda2e21ac365dc9c7d94a5bae5730f9b3";
            const url = 'https://deep-index.moralis.io/api/v2/resolve/'+singleAddress+'/reverse';
            h.fancylog('[ '+chalk.yellow(count)+'/'+chalk.yellow(totalCount)+' ]\tlooking up: '+chalk.magenta(singleAddress), 'ENS');
            ...
            try {
                ...
                console.log('------------------------------------');
                console.log(error.message, error.response.data); // usually means the name is not found on OpenSea if 404 ERR_BAD_REQUEST
                // console.log(Object.keys(error.response.data));
                console.log('------------------------------------');

                db.collection("lookup").updateOne({address: singleAddress }, {$set:{ 'ENS': singleAddress } },{upsert: true},  function(err, result) {
                    if (err) console.log('Mongo ERR: ',err);
                    client.close();
