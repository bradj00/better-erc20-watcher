...
function checkIfAddressesExistInFriendlyNames( uniqueAddys ){
    ...
    return new Promise(async (resolve, reject) => {
        ...
        for (const address of uniqueAddys) {
            ...
            if (collectionExists && typeof collectionExists[0] == 'object' && collectionExists[0].MegaWorld) {
            ...
            return promiseChain.then(() => {
                ...
                if (index == ListOfAddresses.length - 1) {
                    ...
                    MongoClient.connect(mongoUrl, function(err, client) {
                        ...
                        var d = new Date();
                        db.collection("systemStatuses").updateOne({name:"translator-megaworld"}, {$set:{name:"translator-megaworld", lookupIndex: 0, lookupIndexMax: 0, statusMsg: "indexing new addresses against external API", lastAction: d}},{upsert: true},  function(err, result) {
                ...
                MongoClient.connect(mongoUrl, function(err, client) {
                    ...
                    const db = client.db('systemStats');
                    var d = new Date();
                    db.collection("systemStatuses").updateOne({name:"translator-megaworld"}, {$set:{name:"translator-megaworld", lookupIndex: index+1, lookupIndexMax: ListOfAddresses.length, statusMsg: "indexing new addresses against external API", lastAction: d}},{upsert: true},  function(err, result) {
...
const LookupSingleAddress =  (singleAddress, count, totalCount) => {
    return new Promise( (resolve, reject) => {
        setTimeout(async() => {
            const url = 'https://ws.mcp3d.com/user/get';
            h.fancylog('[ '+chalk.yellow(count)+'/'+chalk.yellow(totalCount)+' ]\tlooking up: '+chalk.magenta(singleAddress), 'MegaWorld');
            ...
            try {
                let payload = {"address":singleAddress,"sign":null}
                let data = await axios.post(url, payload)
                // console.log('got: ', data.data);
                console.log('got: ', data.data.avatar_name, ' registered: ',data.data.registered);
                if (data.data.avatar_name == null || data.data.avatar_name == '') { data.data.avatar_name = singleAddress }
                db.collection("lookup").updateOne({address: singleAddress }, {$set:{ 'MegaWorld': data.data.avatar_name }},{upsert: true},  function(err, result) {
                    if (err) console.log('Mongo ERR: ',err);
                    client.close();
                    resolve(data.data.avatar_name);
                });
                ...
                db.collection("lookup").updateOne({address: singleAddress }, {$set:{ 'MegaWorld': singleAddress } },{upsert: true},  function(err, result) {
                    if (err) console.log('Mongo ERR: ',err);
                    client.close();
