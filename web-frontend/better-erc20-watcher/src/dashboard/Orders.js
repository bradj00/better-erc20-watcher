            ...
            {!filteredtxData? txData? txData.map((row, index) => {
              ...
              return(
                ...
                <TableRow className={rowAge > 100? "rowHover": "transactionRow"} style={{fontSize:'3vw', backgroundColor: row.transaction_hash? 'rgba('+(parseInt(row.transaction_hash.substr(0,4), 16) %  30)+', '+(parseInt(row.transaction_hash.substr(5,10), 16) %  30)+', '+(parseInt(row.transaction_hash.substr(12,19), 16) %  30)+', 1)' :'rgba(0,0,0,0)'}} key={index}>
                  ...
                  <TableCell style={{fontSize:'1vw',color: "#aaa"}}  onClick={ ()=>{processTableClicked(row, 'from')} }   >{((row.from_address_friendlyName == undefined) )? getEllipsisTxt(row.from_address, 6): Object.entries(row.from_address_friendlyName).find(([key, value]) => key !== '_id' && !value.startsWith('0x')) ? Object.entries(row.from_address_friendlyName).find(([key, value]) => key !== '_id' && !value.startsWith('0x'))[1] : getEllipsisTxt(row.from_address_friendlyName.address,6)}</TableCell>
                  <TableCell style={{fontSize:'1vw',color: "#aaa"}}      onClick={ ()=>{processTableClicked(row, 'to') } }>   {((row.to_address_friendlyName== undefined) )? getEllipsisTxt(row.to_address, 6): Object.entries(row.to_address_friendlyName).find(([key, value]) => key !== '_id' && !value.startsWith('0x')) ? Object.entries(row.to_address_friendlyName).find(([key, value]) => key !== '_id' && !value.startsWith('0x'))[1] : getEllipsisTxt(row.to_address_friendlyName.address,6)} </TableCell>
